...
static_assert(
    sizeof(message_header) == 14,
    "message_header should be 14 bytes (uint32_t message_type, int8_t mac[6], uint32_t checksum");
static_assert(sizeof(periodicity_message) == 4,
              "struct periodicity_message should be 4 bytes long.");
static_assert(sizeof(sta_wma_lm) == 12, "struct sta_wma_lm should be 12 bytes long");
#endif // __MESSAGES_H